...
def calculate(): 
    ...
    def reciever():
        if request.is_json() is None: # some sanitising here - only allow JSON
            abort(404)
        else:
    ...
    def fitting():
            ...
            result_upper = ps.psignifit(recieved_data,options);
            options['threshPC']    = 0.7;
            result_lower = ps.psignifit(recieved_data,options);
