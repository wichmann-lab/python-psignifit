...
def marginalize(result, dimension):
    ...
    if isinstance(dimension, (int, float)):
        assert dimension in range(
            0, 5
        ), 'the dimensions you want to marginalize to should be given as a number between 0 and 4'
        d = 1
    else:
        assert dimension.all() in range(
            0, 5
        ), 'the dimensions you want to marginalize to should be given as a vector of numbers 0 to 4'
        d = len(result['X1D'])

    if d == 1 and ('marginals' in result.keys()
                  ) and len(result['marginals']) - 1 >= dimension:
        ...
        if not ('Posterior' in result.keys()):
            raise ValueError(
                'marginals cannot be computed anymore because posterior was dropped'
            )
        else:
            ...
            for i in range(0, d):
                if not (any(i == dimension)) and marginal.shape[i] > 1:
                    ...
                    weight = np.sum(weight, i, keepdims=True) / (
                        np.max(result['X1D'][i]) - np.min(result['X1D'][i]))
