...
def psignifit(data, optionsIn):
    ...
    if all(np.logical_and(data[:, 1] <= 1, data[:, 1] >= 0)) and any(
            np.logical_and(data[:, 1] > 0, data[:, 1] < 1)):
        data[:, 1] = round(
            map(lambda x, y: x * y, data[:, 2],
                data[:, 1]))  # we try to convert into our notation
    ...
    if options['sigmoidName'] in ['Weibull', 'logn', 'weibull']:
        ...
        assert min(
            data[:, 0]
        ) > 0, 'The sigmoid you specified is not defined for negative data points!'
    else:
        options['logspace'] = 0
    ...
    if len(np.ravel(options['stimulusRange'])) <= 1:
        if options['logspace']:
            options['stimulusRange'] = np.array(
                np.log([min(data[:, 0]), max(data[:, 0])]))
        else:
            options['stimulusRange'] = np.array(
    ...
    if not ('widthmin' in options.keys()):
        if len(np.unique(data[:, 0])) > 1 and not (stimRangeSet):
            if options['logspace']:
                options['widthmin'] = min(
                    np.diff(np.sort(np.unique(np.log(data[:, 0])))))
            else:
                options['widthmin'] = min(
                    np.diff(np.sort(np.unique(data[:, 0]))))
        else:
    ...
    if (len(np.unique(data[:, 0])) >= 25) and (np.ravel(
            options['stimulusRange']).size == 1):
...
def getSlopePC(result, pCorrect, unscaled=False):
    ...
    if sigName in ['norm', 'gauss', 'neg_norm', 'neg_gauss']:
        ...
        C = _my_norminv(1 - alpha, 0, 1) - _my_norminv(alpha, 0, 1)
        stimLevel = np.exp(
            _my_norminv(pCorrectUnscaled,
                        theta0[0] - _my_norminv(PC, 0, theta0[1] / C),
                        theta0[1] / C))
        normalizedStimLevel = _my_norminv(pCorrectUnscaled, 0, 1)
        slopeNormalized = scipy.stats.norm.pdf(normalizedStimLevel)
        slope = slopeNormalized * C / theta0[1] / stimLevel
...
def getThreshold(result, pCorrect, unscaled=False):
    ...
    if sigName in ['norm', 'gauss', 'neg_norm', 'neg_gauss']:
        C = _my_norminv(1 - alpha, 0, 1) - _my_norminv(alpha, 0, 1)
        threshold = _my_norminv(pCorrectUnscaled,
                                theta0[0] - _my_norminv(PC, 0, theta0[1] / C),
                                theta0[1] / C)
    elif sigName in ['logistic', 'neg_logistic']:
        ...
        C = np.log(-np.log(1 - alpha)) - np.log(-np.log(alpha))
        threshold = theta0[0] + (log(-log(pCorrectUnscaled)) -
                                 log(-log(PC))) * theta0[1] / C
    elif sigName in ['logn', 'neg_logn']:
        C = _my_norminv(1 - alpha, 0, 1) - _my_norminv(alpha, 0, 1)
        threshold = np.exp(
            _my_norminv(pCorrectUnscaled,
        ...
        C = (_my_t1icdf(1 - alpha) - _my_t1icdf(alpha))
        threshold = (_my_t1icdf(pCorrectUnscaled) -
                     _my_t1icdf(PC)) * theta0[1] / C + theta0[0]
    else:
        raise ValueError('unknown sigmoid function')
    ...
    for iConfP in range(0, len(result['options']['confP'])):
        ...
        if sigName in ['norm', 'gauss', 'neg_norm', 'neg_gauss']:
            CI[iConfP, 0] = _my_norminv(
                pCorrMin, thetaMin[0] - _my_norminv(PC, 0, thetaMin[1] / C),
                thetaMin[1] / C)
            CI[iConfP, 1] = _my_norminv(
                pCorrMax, thetaMax[0] - _my_norminv(PC, 0, thetaMax[1] / C),
                thetaMax[1] / C)
        elif sigName in ['logistic', 'neg_logistic']:
            ...
            CI[iConfP, 0] = np.exp(
                _my_norminv(pCorrMin,
                            thetaMin[0] - _my_norminv(PC, 0, thetaMin[1] / C),
                            thetaMin[1] / C))
            CI[iConfP, 1] = np.exp(
                _my_norminv(pCorrMax,
                            thetaMax[0] - _my_norminv(PC, 0, thetaMax[1] / C),
                            thetaMax[1] / C))
        elif sigName in ['Weibull', 'weibull', 'neg_Weibull', 'neg_weibull']:
            CI[iConfP, 0] = np.exp(
            ...
            CI[iConfP, 0] = (_my_t1icdf(pCorrMin) -
                             _my_t1icdf(PC)) * thetaMin[1] / C + thetaMin[0]
            CI[iConfP, 1] = (_my_t1icdf(pCorrMax) -
                             _my_t1icdf(PC)) * thetaMax[1] / C + thetaMax[0]
        else:
            raise ValueError('unknown sigmoid function')
